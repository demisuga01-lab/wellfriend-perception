...
The workspace intentionally has five production crates after MP4:
...
The additional production crates are `reconstruction`, which owns typed canonical reconstruction, planar page mapping, post-warp quality, lens, and surface seams; and `restoration`, which owns condition analysis, deterministic routing, scalar filters, semantic output contracts, and runtime DomainPack registration.
...
`intelligence/src/domains/document.rs` is the first executable reference-domain layer. The root `domains/document/` directory remains the cross-language architecture and asset seam; it does not contain a second implementation. `reconstruction/src/document.rs` is the only MP4 planar document implementation. `restoration/src` must not reimplement perspective geometry: it consumes canonical page images and reconstruction evidence. `restoration/src/domain_pack.rs` registers document behavior and non-document stubs without extending generic core semantics.
