...
for spec in "arm64-v8a:aarch64-linux-android:aarch64-linux-android21-clang:aarch64-linux-android" "x86_64:x86_64-linux-android:x86_64-linux-android21-clang:x86_64-linux-android"; do
  ...
  rustflags_key="CARGO_TARGET_${target^^}_RUSTFLAGS"; rustflags_key="${rustflags_key//-/_}"
  export "$rustflags_key=-C link-arg=-Wl,-z,max-page-size=16384"
  ...
  "$HOST/bin/$clang" -shared -fPIC -I"$HOST/sysroot/usr/include" -I"$HOST/sysroot/usr/include/$arch" -I"$ROOT/bindings/ffi/include" "$ROOT/bindings/android-jni/wellfriend_perception_jni.c" -L"$OUT/$abi" -lwellfriend_perception -Wl,-soname,libwellfriend_perception_jni.so -Wl,-z,max-page-size=16384 -o "$OUT/$abi/libwellfriend_perception_jni.so"
done
"$ROOT/scripts/check-android-page-size.sh" "$OUT" "$NDK_ROOT"
...
manifest={'schema_version':1,'artifact_kind':'wellfriend-android-abi','source_sha':subprocess.check_output(['git','-C',root,'rev-parse','HEAD'],text=True).strip(),'rust_version':subprocess.check_output(['rustc','--version'],text=True).strip(),'ndk_root':ndk,'ndk_version':os.path.basename(ndk),'profile':profile,'page_size_alignment_bytes':16384,'libraries':records,'generated_at_utc':datetime.datetime.now(datetime.timezone.utc).isoformat()}
