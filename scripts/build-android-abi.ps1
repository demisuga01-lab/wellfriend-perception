...
foreach ($Name in $Abi) {
    ...
    & $clang -shared -fPIC "-I$include" "-I$archInclude" "-I$(Join-Path $RepoRoot 'bindings\ffi\include')" $jniSource "-L$abiOutput" -lwellfriend_perception "-Wl,-soname,libwellfriend_perception_jni.so" "-o$(Join-Path $abiOutput 'libwellfriend_perception_jni.so')"
...
$manifest = [ordered]@{ schema_version = 1; artifact_kind = "wellfriend-android-abi"; source_sha = (& git -C $RepoRoot rev-parse HEAD).Trim(); rust_version = (& rustc --version).Trim(); ndk_root = $NdkRoot; ndk_version = Split-Path $NdkRoot -Leaf; profile = $Profile; libraries = $records; generated_at_utc = (Get-Date).ToUniversalTime().ToString("o") }
